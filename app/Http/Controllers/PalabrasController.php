...
class PalabrasController extends Controller
{
    ...
    public function __invoke(Request $request)
    {
        ...
        $palabra = ltrim($request->palabra);


        if($cantidad >2 && $cantidad != null && $palabra != null && strlen($palabra) > $cantidad){
            

            $tiempo_inicio = microtime(true);
        $datos = DB::select('select sin_acentos from palabras where CHARACTER_LENGTH(sin_acentos) = ?', [$cantidad]);
        $salida[] = "";
        $array = str_split($palabra, 1); // 1. Es mejor que haga esto una sola vez.
        ...
        // return ($salida);
            ...
            return "ERROR: Digite un dato válido";
        }       
    }
}
